...
.mh{display:flex;align-items:center;justify-content:space-between;margin-bottom:28px}
...
.btn{display:inline-flex;align-items:center;gap:8px;padding:10px 20px;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;border:none;text-decoration:none;transition:all .2s}
.btn-primary{background:#006AD7;color:#fff}.btn-primary:hover{background:#0057b3}
.btn-sm{padding:6px 12px;font-size:12px;border-radius:7px}
.btn-edit{background:#f0f4ff;color:#006AD7}.btn-edit:hover{background:#dbeafe}
.btn-del{background:#fef2f2;color:#dc2626}.btn-del:hover{background:#fee2e2}
.kg{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:28px}
...
.card{background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:22px;box-shadow:0 1px 4px rgba(0,0,0,.04)}
.filters{display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap}
.filters select,.filters input{padding:8px 12px;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;color:#374151;background:#fff;outline:none}
.filters select:focus,.filters input:focus{border-color:#006AD7}
.dt{width:100%;border-collapse:collapse}
.dt th{text-align:left;font-size:11px;font-weight:600;letter-spacing:.5px;text-transform:uppercase;color:#6b7280;padding:10px 12px;border-bottom:1px solid #e5e7eb;background:#f9fafb}
.dt td{padding:13px 12px;font-size:13px;color:#333;border-bottom:1px solid #f5f5f5;vertical-align:middle}
...
.badge-occupe{background:#f0fdf4;color:#16a34a}
.badge-libre{background:#fffbeb;color:#d97706}
.badge-app{background:#eff6ff;color:#2563eb}
.badge-com{background:#fdf4ff;color:#9333ea}
.badge-par{background:#f0f4ff;color:#006AD7}
.badge-loc{background:#fff7ed;color:#ea580c}
.pag{display:flex;justify-content:center;gap:6px;margin-top:20px}
.pag a,.pag span{padding:6px 12px;border-radius:8px;font-size:13px;border:1px solid #e5e7eb;color:#374151;text-decoration:none}
.pag .active-p{background:#006AD7;color:#fff;border-color:#006AD7}
</style>
@endpush
@section('content')
<div class="dw">
...
<aside class="sb">
  <nav class="sb-nav">
    <a href="{{ route('syndic.dashboard') }}"       class="sb-item {{ request()->routeIs('syndic.dashboard') ? 'active' : '' }}">📊 Tableau de bord</a>
    <a href="{{ route('syndic.coproprietes.index') }}" class="sb-item {{ request()->routeIs('syndic.coproprietes*') ? 'active' : '' }}">🏙️ Copropriétés</a>
    <a href="{{ route('syndic.lots.index') }}"      class="sb-item {{ request()->routeIs('syndic.lots*') ? 'active' : '' }}">🏠 Lots</a>
    <span class="sb-item disabled">👥 Résidents    <span class="badge-soon">bientôt</span></span>
    <span class="sb-item disabled">📄 Factures     <span class="badge-soon">bientôt</span></span>
    <span class="sb-item disabled">📢 Réclamations <span class="badge-soon">bientôt</span></span>
    <span class="sb-item disabled">🔧 Fournisseurs <span class="badge-soon">bientôt</span></span>
...
<main class="mc">
  ...
  @if(session('success'))
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;color:#16a34a;padding:12px 16px;border-radius:10px;margin-bottom:20px;font-size:14px;">✅ {{ session('success') }}</div>
  @endif
  @if(session('error'))
    <div style="background:#fef2f2;border:1px solid #fecaca;color:#dc2626;padding:12px 16px;border-radius:10px;margin-bottom:20px;font-size:14px;">❌ {{ session('error') }}</div>
  @endif

  {{-- Stats --}}
  <div class="kg">
    <div class="kc"><div class="ki b">🏠</div><div><div class="kn">{{ $stats['total'] }}</div><div class="kl">Total lots</div></div></div>
    <div class="kc"><div class="ki g">✅</div><div><div class="kn">{{ $stats['occupes'] }}</div><div class="kl">Occupés</div></div></div>
    <div class="kc"><div class="ki o">🔓</div><div><div class="kn">{{ $stats['libres'] }}</div><div class="kl">Libres</div></div></div>
  </div>

  <div class="card">
    {{-- Filtres --}}
    <form method="GET" class="filters">
      <input type="text" name="search" placeholder="🔍 Rechercher un lot..." value="{{ request('search') }}" style="flex:1;min-width:200px">
      <select name="type">
        <option value="">Tous les types</option>
        <option value="appartement" {{ request('type')=='appartement'?'selected':'' }}>Appartement</option>
        <option value="commerce"    {{ request('type')=='commerce'?'selected':'' }}>Commerce</option>
      ...
      <button type="submit" class="btn btn-primary btn-sm">Filtrer</button>
      <a href="{{ route('syndic.lots.index') }}" class="btn btn-sm" style="background:#f3f4f6;color:#374151">Réinitialiser</a>
    </form>
    ...
    <table class="dt">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            @php
              $tc = ['appartement'=>'badge-app','commerce'=>'badge-com','parking'=>'badge-par','local'=>'badge-loc'];
              $ti = ['appartement'=>'🏢','commerce'=>'🏪','parking'=>'🚗','local'=>'🏭'];
            @endphp
            <span class="badge {{ $tc[$lot->type] ?? '' }}">{{ $ti[$lot->type] ?? '' }} {{ ucfirst($lot->type) }}</span>
          </td>
          <td>{{ number_format($lot->surface, 2) }} m²</td>
          <td>{{ number_format($lot->quote_part * 100, 2) }}%</td>
          <td>
