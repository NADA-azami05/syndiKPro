...
.back{display:inline-flex;align-items:center;gap:6px;color:#6b7280;font-size:13px;text-decoration:none;margin-bottom:20px;transition:color .2s}
.back:hover{color:#006AD7}
...
.card{background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:32px;box-shadow:0 1px 4px rgba(0,0,0,.04);max-width:700px}
...
.fg input,.fg select{padding:10px 14px;border:1px solid #e5e7eb;border-radius:9px;font-size:14px;color:#111;outline:none;transition:border-color .2s;background:#fff}
.fg input:focus,.fg select:focus{border-color:#006AD7;box-shadow:0 0 0 3px rgba(0,106,215,.08)}
.fg .hint{font-size:11px;color:#9ca3af}
.err{font-size:11px;color:#dc2626;margin-top:2px}
.actions{display:flex;gap:12px;margin-top:8px}
.btn{display:inline-flex;align-items:center;gap:8px;padding:11px 24px;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;border:none;text-decoration:none;transition:all .2s}
...
.btn-del{background:#fef2f2;color:#dc2626;border:1px solid #fecaca}.btn-del:hover{background:#fee2e2}
.section-title{font-size:13px;font-weight:700;color:#006AD7;text-transform:uppercase;letter-spacing:.5px;margin-bottom:16px;padding-bottom:8px;border-bottom:1px solid #e5e7eb}
.info-box{background:#f0f4ff;border:1px solid #bfdbfe;border-radius:10px;padding:12px 16px;margin-bottom:24px;font-size:13px;color:#1e40af}
</style>
@endpush
@section('content')
<div class="dw">
...
<main class="mc">
  ...
  <div class="card">
      ...
      <div class="grid2">
        ...
        <div class="fg" style="grid-column:span 2">
          ...
          <select name="copropriete_id" id="copropriete_id" required>
            ...
            @foreach($coproprietes as $c)
              <option value="{{ $c->id }}"
                {{ old('copropriete_id', $lot->copropriete_id) == $c->id ? 'selected' : '' }}>
                {{ $c->nom }} — {{ $c->ville }}
              </option>
            @endforeach
          </select>
        ...
        <div class="fg">
          <label for="type">Type *</label>
          <select name="type" id="type" required>
            <option value="appartement" {{ old('type',$lot->type)=='appartement'?'selected':'' }}>🏢 Appartement</option>
            <option value="commerce"    {{ old('type',$lot->type)=='commerce'?'selected':'' }}>🏪 Commerce</option>
            <option value="parking"     {{ old('type',$lot->type)=='parking'?'selected':'' }}>🚗 Parking</option>
            <option value="local"       {{ old('type',$lot->type)=='local'?'selected':'' }}>🏭 Local</option>
          </select>
          @error('type')<span class="err">{{ $message }}</span>@enderror
        </div>
        ...
        <div class="fg">
          <label for="statut">Statut *</label>
          <select name="statut" id="statut" required>
            <option value="libre"  {{ old('statut',$lot->statut)=='libre'?'selected':'' }}>🔓 Libre</option>
            <option value="occupe" {{ old('statut',$lot->statut)=='occupe'?'selected':'' }}>✅ Occupé</option>
          </select>
          @error('statut')<span class="err">{{ $message }}</span>@enderror
        </div>
      </div>
    ...
    {{-- Suppression --}}
    <div style="margin-top:32px;padding-top:20px;border-top:1px solid #fee2e2">
      <p style="font-size:13px;color:#6b7280;margin-bottom:12px">⚠️ Zone dangereuse — cette action est irréversible.</p>
      <form action="{{ route('syndic.lots.destroy', $lot) }}" method="POST"
            onsubmit="return confirm('Supprimer définitivement le lot {{ $lot->numero }} ?')">
        @csrf @method('DELETE')
        <button type="submit" class="btn btn-del">🗑️ Supprimer ce lot</button>
