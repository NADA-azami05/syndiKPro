...
.kg{display:grid;grid-template-columns:repeat(5,1fr);gap:14px;margin-bottom:8px}
.kc{background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:20px 18px;display:flex;align-items:center;gap:14px;transition:box-shadow .2s,transform .2s}
.kc:hover{box-shadow:0 4px 20px rgba(0,106,215,.1);transform:translateY(-2px)}
.ki{width:46px;height:46px;border-radius:12px;display:grid;place-items:center;font-size:20px;flex-shrink:0}
.ki.b{background:#e6f2ff}.ki.g{background:#f0fdf4}.ki.o{background:#fffbeb}.ki.r{background:#fef2f2}.ki.p{background:#f5f3ff}
...
.cg3{display:grid;grid-template-columns:2fr 1fr;gap:16px}
.dc{background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:22px;box-shadow:0 1px 4px rgba(0,0,0,.04)}
...
.bp{background:#f0fdf4;color:#16a34a}.be{background:#eff6ff;color:#2563eb}.br{background:#fef2f2;color:#dc2626}.bb{background:#f9fafb;color:#6b7280}.ba{background:#fffbeb;color:#d97706}.bn{background:#f0f4ff;color:#006AD7}.bu{background:#fef2f2;color:#dc2626}.bc{background:#fdf4ff;color:#9333ea}
.pw{margin-bottom:14px}
.pl{display:flex;justify-content:space-between;font-size:13px;color:#444;margin-bottom:6px}
.pb{height:8px;background:#e5e7eb;border-radius:100px;overflow:hidden}
.pf{height:100%;border-radius:100px;transition:width .5s}
.pf.g{background:#22c55e}.pf.o{background:#f59e0b}
.fa{font-family:var(--font-serif);font-size:32px;font-weight:700;color:#22c55e;text-align:center;padding:12px 0 16px;line-height:1}
.fl{font-size:13px;color:#6b7280;text-align:center;margin-bottom:16px}
.fi{display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid #f5f5f5}
.fi:last-child{border-bottom:none}
.fav{width:38px;height:38px;border-radius:9px;background:#e6f2ff;display:grid;place-items:center;font-size:17px;flex-shrink:0}
.fn{font-size:13px;font-weight:500;color:#111}
.fc{font-size:11px;color:#6b7280;margin-top:1px}
.st{color:#f59e0b;font-size:11px}
...
<main class="mc">
  ...
  @if(session('success'))
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;color:#16a34a;padding:12px 16px;border-radius:10px;margin-bottom:20px;font-size:14px;">
      ✅ {{ session('success') }}
    </div>
  @endif
  @if(session('error'))
    <div style="background:#fef2f2;border:1px solid #fecaca;color:#dc2626;padding:12px 16px;border-radius:10px;margin-bottom:20px;font-size:14px;">
      ❌ {{ session('error') }}
    </div>
  @endif
  ...
  <div class="kg">
    <div class="kc"><div class="ki b">👥</div><div><div class="kn">{{ $nb_residents }}</div><div class="kl">Résidents actifs</div></div></div>
    <div class="kc"><div class="ki o">📄</div><div><div class="kn">{{ $nb_factures_attente }}</div><div class="kl">Factures en attente</div></div></div>
    <div class="kc"><div class="ki r">📢</div><div><div class="kn">{{ $nb_reclamations }}</div><div class="kl">Réclamations</div></div></div>
    <div class="kc"><div class="ki g">💰</div><div><div class="kn">{{ number_format($total_recouvre/1000,1) }}k</div><div class="kl">Recouvré (MAD)</div></div></div>
    <div class="kc"><div class="ki p">🔧</div><div><div class="kn">{{ $nb_fournisseurs }}</div><div class="kl">Fournisseurs</div></div></div>
  </div>
  ...
  <div class="cg">
    <div class="dc">
      ...
      <table class="dt">
        <thead><tr><th>Numéro</th><th>Résident</th><th>Total</th><th>Statut</th></tr></thead>
        <tbody>
          ...
          <tr>
            ...
            <td>
              @php
                $bl=['payee'=>'bp','envoyee'=>'be','retard'=>'br','brouillon'=>'bb','en_attente_confirmation'=>'ba'];
                $ll=['payee'=>'✓ Payée','envoyee'=>'📤 Envoyée','retard'=>'⚠ Retard','brouillon'=>'📝 Brouillon','en_attente_confirmation'=>'⏳ Confirm.'];
              @endphp
              <span class="b {{ $bl[$f->statut]??'bb' }}">{{ $ll[$f->statut]??$f->statut }}</span>
            </td>
          </tr>
          @empty
          <tr><td colspan="4" style="text-align:center;color:#6b7280;padding:16px">Aucune facture</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
      ...
      <table class="dt">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><span class="b @if($r->priorite=='urgente') bu @elseif($r->priorite=='critique') bc @else bn @endif">{{ ucfirst($r->priorite) }}</span></td>
            <td><span class="b ba">⏳ En attente</span></td>
          </tr>
          @empty
          <tr><td colspan="3" style="text-align:center;color:#6b7280;padding:16px">Aucune réclamation</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
  ...
  <div class="cg3">
    <div class="dc">
      ...
      <div class="pw"><div class="pl"><span>Taux de recouvrement</span><span><strong>{{ $pct }}%</strong></span></div><div class="pb"><div class="pf g" style="width:{{ $pct }}%"></div></div></div>
      <div class="pw"><div class="pl"><span>En attente</span><span><strong>{{ number_format($tE,0,',',' ') }} MAD</strong></span></div><div class="pb"><div class="pf o" style="width:{{ $pctA }}%"></div></div></div>
    </div>
